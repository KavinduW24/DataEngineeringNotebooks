...
errors = logs.filter(lambda x:x.split()[2] =="ERROR")# YOUR CODE
...
levels = logs.map(lambda x:x.split()[2]) # YOUR CODE (hint: split and take index 2)
...
error_messages = errors.map(lambda x:x.split("ERROR")[1]) # YOUR CODE
...
sorted_counts = word_counts.sortBy(lambda x: x[1],ascending=False)# YOUR CODE (sort by count, descending)
...
pipeline = web_logs.filter(lambda x: x.split()[3] == "200").map(lambda x: (x.split()[2],1)).reduceByKey(lambda a, b: a + b).sortBy(lambda x:x[1], ascending=False)
print(f"Final Pipeline{pipeline.collect()}")
